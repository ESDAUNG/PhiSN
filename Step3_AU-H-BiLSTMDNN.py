...
parser = argparse.ArgumentParser(description="Hybrid-DNN S-BiLSTM N-DNN Training Main - All")
...
cur_folder_path = 'Hybrid_DNNBiLSTMDNN_All_ModelTrainTest/'
...
def Hybrid_DNNBiLSTMDNN_Model(param_dict):
    ### Hybrid Parameters >>>
    ...
    url_bilstm_embed_dim=param_dict['url_bilstm_embed_dim']
    url_bilstm_num_layers=param_dict['url_bilstm_num_layers']
    url_bilstm_num_units=param_dict['url_bilstm_num_units']
    url_bilstm_dropout_rate=param_dict['url_bilstm_dropout_rate']
    url_bilstm_activation=param_dict['url_bilstm_activation']
    url_bilstm_loss=param_dict['url_bilstm_loss']
    url_bilstm_initializer=param_dict['url_bilstm_initializer']
    #url_bilstm_optimizer=param_dict['url_bilstm_optimizer']
    #url_bilstm_learning_rate=param_dict['url_bilstm_learning_rate']
    ...
    ### Initializing S-BiLSTM Model >>>
    if url_bilstm_num_layers != len([url_bilstm_num_units]):
        url_bilstm_num_nodes = [url_bilstm_num_units][0]
        url_bilstm_num_units = [i for i in range(url_bilstm_num_layers) for i in [url_bilstm_num_nodes]]
    url_inputs = layers.Input(shape=(url_max_len,), name='word_BiLSTM_Input_Layer')
    url_bilstm_x = TokenAndPositionEmbedding.TokenAndPositionEmbedding(url_max_len, url_vocab_size, url_bilstm_embed_dim)(url_inputs)
    url_bilstm_x = layers.BatchNormalization()(url_bilstm_x)
    url_bilstm_x = layers.Dropout(url_bilstm_dropout_rate)(url_bilstm_x)
    for i in range(url_bilstm_num_layers):
        url_bilstm_x = layers.Bidirectional(layers.LSTM(units=url_bilstm_num_units[i], return_sequences=True,kernel_initializer=url_bilstm_initializer, dropout=url_bilstm_dropout_rate))(url_bilstm_x)
    url_bilstm_x = layers.Flatten()(url_bilstm_x)
    url_bilstm_x = layers.Dense(units=8, activation=url_bilstm_activation)(url_bilstm_x)
    url_bilstm_outputs = layers.Dense( units=1, name='url_bilstm_outputs', activation=activation_out)(url_bilstm_x)
    ...
    ### H-DNN: Concatenate Outputs from S-BiLSTM and N-DNN >>>
    dnn_bilstm_dnn_x = layers.Concatenate(axis=1)([nlp_dnn_x,url_bilstm_x])
    ### H-DNN: Normalize Layer >>>
    dnn_bilstm_dnn_x = layers.BatchNormalization()(dnn_bilstm_dnn_x)
    ### H-DNN: Dense Layer >>>
    for i in range(num_layers):
        dnn_bilstm_dnn_x = layers.Dense(units=num_units[i], activation=activation,kernel_initializer=initializer, kernel_regularizer=regularizers.L1L2())(dnn_bilstm_dnn_x)
    dnn_bilstm_dnn_outputs = layers.Dense(units=1, name='dnn_bilstm_dnn_outputs', activation=activation_out, kernel_initializer=initializer, kernel_regularizer=regularizers.L1L2())(dnn_bilstm_dnn_x)
    
    ###### Initializing H-DNN_S-BiLSTM_N-DNN Model #################
    dnn_bilstm_dnn_model = Model(inputs=[nlp_inputs, url_inputs], outputs=[nlp_dnn_outputs,url_bilstm_outputs,dnn_bilstm_dnn_outputs])
    ###### H-DNN Optimizer #################
    ...
    ###### Compiling H-DNN #################
    dnn_bilstm_dnn_model.compile(loss={'nlp_dnn_outputs': nlp_dnn_loss, 'url_bilstm_outputs': url_bilstm_loss, 'dnn_bilstm_dnn_outputs': loss}, optimizer=optimizer, metrics=metrics)
    dnn_bilstm_dnn_model.summary()

    return dnn_bilstm_dnn_model
...
def build_model(param_dict:dict):
    model = Hybrid_DNNBiLSTMDNN_Model(param_dict)
    return model
...
def build_eval(X_train_data,X_train_data4Domain,y_train,X_valid_data4url,y_valid_url,X_valid_data4domain,y_valid_domain,X_valid_data4full,y_valid,param_dict,param_dict4domain,result_folder_path,model_id):
    ...
    ##### Slicing Concatenated data into S-BiLSTM and N-DNN data
    ...
    stop_early = tf.keras.callbacks.EarlyStopping(monitor='dnn_bilstm_dnn_outputs_loss', patience=50,restore_best_weights=True)
    ...
    model_file = result_folder_path+'url-n-dnn_s-bilstm_{}.h5'.format(model_id)
    model_file = Path(model_file)
    print('Model file ',model_file)
    if model_file.is_file():
        print('URL Model existed!!!')
        model = tf.keras.models.load_model(result_folder_path+'url-n-dnn_s-bilstm_{}.h5'.format(model_id), custom_objects={'TokenAndPositionEmbedding': TokenAndPositionEmbedding.TokenAndPositionEmbedding})
    else:
        ...
        model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(filepath=checkpoint_filepath_model_id+'/checkpoint',save_weights_only=True,monitor='dnn_bilstm_dnn_outputs_accuracy',mode='max',save_best_only=True)    
        t1 = time()
        ...
        print('Model was saved to {}url-n-dnn_s-bilstm_{}.h5'.format(result_folder_path,model_id))
        model.save(result_folder_path+'url-n-dnn_s-bilstm_{}.h5'.format(model_id))

        val_acc_per_epoch=history.history['dnn_bilstm_dnn_outputs_accuracy']
        best_epoch=val_acc_per_epoch.index(max(val_acc_per_epoch))+1
        model.save(result_folder_path+'url-n-dnn_s-bilstm_{}@BestEpoch_{}'.format(model_id,best_epoch),save_format='tf')

    tf.keras.backend.clear_session()

    ##### Training for Domain-Level Model: to be trained with URLs w/o Domain
    stop_early_domain = tf.keras.callbacks.EarlyStopping(monitor='dnn_bilstm_dnn_outputs_loss', patience=50,restore_best_weights=True)
    ...
    model_file = result_folder_path+'domain-n-dnn_s-bilstm_{}.h5'.format(model_id)
    model_file = Path(model_file)
    if model_file.is_file():
        print('Domain Model existed!!!')
        model4Domain = tf.keras.models.load_model(result_folder_path+'domain-n-dnn_s-bilstm_{}.h5'.format(model_id), custom_objects={'TokenAndPositionEmbedding': TokenAndPositionEmbedding.TokenAndPositionEmbedding})
    else:
        ...
        model_checkpoint_callback4domain = tf.keras.callbacks.ModelCheckpoint(filepath=checkpoint_filepath4Domain_model_id+'/checkpoint',save_weights_only=True,monitor='dnn_bilstm_dnn_outputs_accuracy',mode='max',save_best_only=True)
        t1 = time()
        ...
        print('Model was saved to {}domain-n-dnn_s-bilstm_{}.h5'.format(result_folder_path,model_id))
        model4Domain.save(result_folder_path+'domain-n-dnn_s-bilstm_{}.h5'.format(model_id))

        val_acc_per_epoch=history4domain.history['dnn_bilstm_dnn_outputs_accuracy']
        best_epoch=val_acc_per_epoch.index(max(val_acc_per_epoch))+1
        model4Domain.save(result_folder_path+'domain-n-dnn_s-bilstm_{}@BestEpoch_{}'.format(model_id,best_epoch),save_format='tf')

    ##### Testing on URL-Level (URLs w/ Path) H-DNN Model
    tf.keras.backend.clear_session()
    print('Model evaluating URL [URLs w/ Path] from {}url-n-dnn_s-bilstm_{}.h5'.format(result_folder_path,model_id))
    t3 = time()
    ...
    tf.keras.backend.clear_session()
    print('Model evaluating Domain [URLs w/o Path] from {}domain-n-dnn_s-bilstm_{}.h5'.format(result_folder_path,model_id))
    t3 = time()
    ...
    tf.keras.backend.clear_session()
    print('Model evaluating ALL [URLs w/ and w/o Path] from {}url-n-dnn_s-bilstm_{}.h5'.format(result_folder_path,model_id))
    t3 = time()
    ...
    predicted_nlp_dnn,predicted_url_bilstm,predicted_dnnbilstmdnn=model.predict(x=[X_valid_nlp_url,X_valid_url],verbose=1)
    y_pre_nlp_dnn = [item for sublist in predicted_nlp_dnn for item in sublist]
    y_pre_url_bilstm = [item for sublist in predicted_url_bilstm for item in sublist]
    y_pre_dnnbilstmdnn = [item for sublist in predicted_dnnbilstmdnn for item in sublist]
    predicted_df = pd.DataFrame()
    predicted_df['Predicted-N-DNN'] = y_pre_nlp_dnn
    predicted_df['Predicted-S-BiLSTM'] = y_pre_url_bilstm
    predicted_df['Predicted-N-DNN_S-BiLSTM_Hybrid-DNN'] = y_pre_dnnbilstmdnn
    ...
    predicted_nlp_dnn,predicted_url_bilstm,predicted_dnnbilstmdnn=model4Domain.predict(x=[X_valid_nlp_domain,X_valid_domain],verbose=1)
    y_pre_nlp_dnn = [item for sublist in predicted_nlp_dnn for item in sublist]
    y_pre_url_bilstm = [item for sublist in predicted_url_bilstm for item in sublist]
    y_pre_dnnbilstmdnn = [item for sublist in predicted_dnnbilstmdnn for item in sublist]
    predicted_df = pd.DataFrame()
    ...
    predicted_nlp_dnn,predicted_url_bilstm,predicted_dnnbilstmdnn=model.predict(x=[X_valid_nlp_full,X_valid_full],verbose=1)
    y_pre_nlp_dnn = [item for sublist in predicted_nlp_dnn for item in sublist]
    y_pre_url_bilstm = [item for sublist in predicted_url_bilstm for item in sublist]
    y_pre_dnnbilstmdnn = [item for sublist in predicted_dnnbilstmdnn for item in sublist]
    predicted_df = pd.DataFrame()
...
for indx in range(n_fold):
    ...
    param_dict_defaults = get_defaults(nlp_dnn_num_layers=2, nlp_dnn_num_units=64, nlp_dnn_dropout_rate=0.1, nlp_dnn_activation='relu',  nlp_dnn_initializer='random_uniform', nlp_dnn_loss='binary_crossentropy', nlp_dnn_optimizer='Adamax', nlp_dnn_learning_rate=0.001, nlp_dnn_epoch_size=100, nlp_dnn_batch_size=64,
                            url_bilstm_embed_dim=128, url_bilstm_num_layers=2, url_bilstm_num_units=32, url_bilstm_dropout_rate=0.5, url_bilstm_activation='relu', url_bilstm_initializer='TruncatedNormal', url_bilstm_loss='binary_crossentropy', url_bilstm_optimizer='Adamax', url_bilstm_learning_rate=0.001, url_bilstm_epoch_size=100, url_bilstm_batch_size=64,nlp_featlen=X_train_nlp_features.shape[-1],class_weights=class_weights)
    param_dict_defaults4domain = get_defaults(nlp_dnn_num_layers=2, nlp_dnn_num_units=64, nlp_dnn_dropout_rate=0.1, nlp_dnn_activation='relu',  nlp_dnn_initializer='TruncatedNormal', nlp_dnn_loss='binary_crossentropy', nlp_dnn_optimizer='Adamax', nlp_dnn_learning_rate=0.001, nlp_dnn_epoch_size=100, nlp_dnn_batch_size=64,
                            url_bilstm_embed_dim=32, url_bilstm_num_layers=8, url_bilstm_num_units=32, url_bilstm_dropout_rate=0.2, url_bilstm_activation='relu', url_bilstm_initializer='glorot_uniform', url_bilstm_loss='binary_crossentropy', url_bilstm_optimizer='Adamax', url_bilstm_learning_rate=0.001, url_bilstm_epoch_size=100, url_bilstm_batch_size=64,nlp_featlen=X_train_nlp_features4Domain.shape[-1],class_weights=class_weights)
